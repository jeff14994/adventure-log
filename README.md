...
橫跨 **美國 / 越南 / 台灣** 的九則驚險經歷，用一張 **d3.js 互動世界地圖** 串起每一次心跳加速的瞬間。
...
```
adventure-log/
├── index.html          首頁：d3 世界地圖 + 故事卡片牆 + 分類篩選
├── story.html          內文頁樣板（以 ?id= 決定顯示哪一則）
├── css/blog.css        暗色主題
├── js/
│   ├── data.js         ★ 唯一資料來源：九則故事 + 分類定義
│   ├── map.js          d3 世界地圖（geoNaturalEarth1 + 可點擊標記）
│   ├── home.js         首頁邏輯：卡片牆、d3 驚險指數計量條、篩選、統計
│   └── story.js        內文頁邏輯：讀 ?id=、d3 正射投影迷你地圖、上/下篇
└── vendor/
    ├── d3.v7.min.js
